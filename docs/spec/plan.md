...
그대로 남아 팀 공유 가능). 프로젝트 밖(projectless)에서의 독립 추가는 레지스트리에 `standalone`으로
기록하고 즉시 active route로 취급한다. 데몬이 떠 있으면 `add`/`rm`이 hot reload하고, 데몬이
멈춰 있으면 다음 `prx daemon start`가 active standalone route를 읽어 올린다.
