...
// logger receives debug output when WithLogger is set; otherwise the standard
// logger is used. This is a generic seam (it depends only on *log.Logger), so
// the package stays self-contained and reusable.
...
// WithLogger routes debug output to l instead of the standard logger. Combined
// with WithDebug it lets a caller capture trust-store activity in its own log
// stream without this package importing anything caller-specific.
