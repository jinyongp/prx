...
flowchart TD
    client["브라우저 / 모바일"]
    prx["prx 데몬 :443 / :80<br/>TLS 종료 · Host 헤더 라우팅"]
    dev["dev 서버<br/>(Vite / Nuxt / Nest / ...)"]
...
flowchart TB
    subgraph prx["prx — 단일 Go 바이너리"]
        subgraph cp["Control plane (CLI)"]
            ...
            exp["expose provider (cloudflared 등)"]
        end
        subgraph dp["Data plane (데몬)"]
            proxy["리버스 프록시 (stdlib) :443 / :80<br/>무중단 reload"]
        end
    end
    cp -.설정·인증서·라우트.-> dp
```
